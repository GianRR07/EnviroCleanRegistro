...
## 9. Corrección recomendada usando la relación `idUser`

La colección actual `records` ya contiene el atributo `idUser`. La versión corregida de la app utiliza este atributo como fuente principal para identificar al trabajador.

Configuración recomendada en Appwrite:

| Configuración | Valor |
|---|---|
| Colección origen | `records` |
| Atributo | `idUser` |
| Tipo | Relationship |
| Colección relacionada | `app_users` |
| Relación | Many-to-one o one-way hacia `app_users` |
| Requerido | No |
| On delete | Set null |

Al crear o actualizar un registro, la app envía en `idUser` el `$id` del documento del trabajador en `app_users`.

Los atributos `trabajadorId`, `trabajadorUsuario` y `trabajadorNombre` siguen siendo compatibles como copia informativa, pero ya no son indispensables. Si no existen, la app conserva la relación `idUser` y el panel administrador obtiene el nombre desde `app_users`.

Importante: los documentos antiguos que ya tienen `idUser: null` no pueden asociarse automáticamente porque no contienen información suficiente para saber quién los creó. Deben vincularse manualmente en Appwrite cuando se conozca el trabajador correspondiente.
