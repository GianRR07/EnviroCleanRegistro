import { saveRecordAppwrite, updateServiceRecordAppwrite } from "@/utils/appwriteRecords";
...
export default function Worker() {
  ...
  const loadDraft = async () => {
    try {
      ...
      if (savedDraft) {
        ...
        if (parsed?.currentWorker) {
          setCurrentWorker(parsed.currentWorker);
        }
      }
    } catch (error) {
  ...
  const saveRegister = async () => {
    ...
    try {
      // Crear un mapa numerado de estaciones para almacenar en Appwrite.
      ...
      // Función auxiliar que valida si el registro está completo.
      ...
      const sessionWorker = currentWorker ?? (await getCurrentUserSession());
      const workerInfo = sessionWorker
        ? {
            id: sessionWorker.appwriteId || sessionWorker.id || "",
            appwriteId: sessionWorker.appwriteId || sessionWorker.id || "",
            username: sessionWorker.username || "",
            name: sessionWorker.name || sessionWorker.username || "Trabajador no identificado",
          }
        : {
            id: "",
            appwriteId: "",
            username: "",
            name: "Trabajador no identificado",
          };

      if (!currentWorker && sessionWorker) {
      ...
      console.log(
        currentAppwriteRecordId
          ? "♻️ Actualizando registro pendiente existente"
          : "✅ Creando nuevo registro",
        record,
      );

      let appwriteRecordId = currentAppwriteRecordId;

      if (currentAppwriteRecordId) {
        console.log("➡️ Actualizando en Appwrite...");
        const response = await updateServiceRecordAppwrite(
          currentAppwriteRecordId,
          record,
        );
        appwriteRecordId = response.$id;
        record = {
          ...record,
          appwriteId: response.$id,
        };
        console.log("✅ Actualizado en Appwrite");
      } else {
        console.log("➡️ Guardando en Appwrite...");
        const response = await saveRecordAppwrite(record);
        appwriteRecordId = response.$id;
        record = {
          ...record,
          appwriteId: response.$id,
      ...
      // Guardar o actualizar localmente en AsyncStorage sin duplicar registros.
      console.log("➡️ Guardando local...");
      const existing = await AsyncStorage.getItem(RECORDS_STORAGE_KEY);
      const records = existing ? JSON.parse(existing) : [];
      const existingIndex = records.findIndex((item: any) => {
        return (
          String(item.id) === String(localRecordId) ||
          (!!appwriteRecordId && String(item.appwriteId) === String(appwriteRecordId))
        );
      });
      ...
      setCurrentAppwriteRecordId(appwriteRecordId ?? null);
      ...
      await AsyncStorage.setItem(
        ...
        JSON.stringify({
          ...
          currentAppwriteRecordId: appwriteRecordId ?? null,
      ...
      Alert.alert(
        recordStatus === "completado" ? "Registro completado" : "Registro pendiente",
        recordStatus === "completado"
          ? "El registro fue actualizado y marcado como completado. Usa 'Cerrar y limpiar para nuevo registro' para iniciar otro servicio."
          : "El registro fue guardado como pendiente. Cuando tomes las fotos faltantes, presiona 'Actualizar registro pendiente' para completar este mismo registro sin duplicarlo.",
      );
    } catch (error: any) {
      ...
      Alert.alert(
        "Error",
        translateAppwriteError(error, "No se pudo guardar el registro."),
      );
    } finally {
      ...
      >
        ...
        {currentRecordStatus && (
          <View
            style={{
              backgroundColor:
                currentRecordStatus === "completado" ? "#E8F5E9" : "#FFF8E1",
              borderRadius: 14,
              padding: 14,
              marginBottom: 14,
              borderWidth: 1,
              borderColor:
                currentRecordStatus === "completado" ? "#A5D6A7" : "#FFE082",
            }}
          >
            <Text
              style={{
                color:
                  currentRecordStatus === "completado" ? "#2E7D32" : "#F57F17",
                fontWeight: "700",
              }}
            >
            ...
            <Text style={{ color: "#555", marginTop: 6 }}>
              {currentRecordStatus === "completado"
                ...
                : "Este registro ya existe como pendiente. Al guardar de nuevo, se actualizará el mismo registro y no se creará un duplicado."}
            </Text>
          </View>
        )}
        ...
        <TouchableOpacity
          style={[
            styles.saveBtn,
            (savingRecord || currentRecordStatus === "completado") && {
              opacity: 0.55,
            },
          ]}
          onPress={saveRegister}
          disabled={savingRecord || currentRecordStatus === "completado"}
        >
          <Ionicons
